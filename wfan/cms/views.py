...
def insert():
    ...
    tweetsDra = tweet_search('中日ドラゴンズ -rt -bot until:2016-02-18 since:2016-02-13',id_Dra["max"], oath_key_dict)
    id_Swa = twitSwa.objects.aggregate(max=Min('twit_id'))
    tweetsSwa = tweet_search('ヤクルトスワローズ -rt -bot until:2016-02-18 since:2016-02-13',id_Swa["max"], oath_key_dict)
    id_Gia = twitGia.objects.aggregate(max=Min('twit_id'))
    tweetsGia = tweet_search('読売ジャイアンツ -rt -bot until:2016-02-18 since:2016-02-13',id_Gia["max"], oath_key_dict)
    id_Tig = twitTig.objects.aggregate(max=Min('twit_id'))
    tweetsTig = tweet_search('阪神タイガース -rt -bot until:2016-02-18 since:2016-02-13',id_Tig["max"], oath_key_dict)
    id_Car = twitCar.objects.aggregate(max=Min('twit_id'))
    tweetsCar = tweet_search('広島カープ -rt -bot until:2016-02-18 since:2016-02-13',id_Car["max"], oath_key_dict)
    id_Bay = twitBay.objects.aggregate(max=Min('twit_id'))
    tweetsBay = tweet_search('横浜ベイスターズ -rt -bot until:2016-02-18 since:2016-02-13',id_Bay["max"], oath_key_dict)
