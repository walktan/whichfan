...
def for_hourly(req):
        ...
        # 開始、終了時間が未入力の場合、直近の12時間を取得
            ...
            from_time = datetime.now()\
                            .replace(minute=0,
                                     second=0,
                                     microsecond=0) - timedelta(hours=11)
        ...
        for var in range(0, diff_time+1):
            for team in TeamMst.objects.values('team_name')\
                    .order_by('team_id'):
                countTweet = TweetTbl.objects\
                    .filter(team_name_id=team['team_name'],
                            twit_at__range=(from_time,
                                            from_time +
                                            timedelta(hours=1))).count()
                countAll.append((team['team_name'], countTweet))
            sort_countall = OrderedDict(countAll)
            link_time.append({'twiDate': from_time.strftime('%-m/%-d %-H:%M'),
                              'twicnt': sort_countall})
...
def for_daily(req):
        ...
        for var in range(0, diff_time+1):
            for team in TeamMst.objects.values('team_name')\
                ...
                countAll.append((team['team_name'], countTweet))
            sort_countall = OrderedDict(countAll)
            link_time.append({'twiDate': from_time
                             .strftime('%-m/%-d') + string.whitespace,
                              'twicnt': sort_countall})
